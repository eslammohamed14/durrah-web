...
function formatPrice(price: number, currency: string, priceType: Property['pricing']['priceType'], t: (k: string) => string): string {
  const formatted = new Intl.NumberFormat('en-SA', { style: 'decimal', maximumFractionDigits: 0 }).format(price);
  const currencyLabel = t(`currency.${currency}`) || currency;
  const suffix = priceType === 'per_night' ? `/${t('property.perNight')}` : priceType === 'per_month' ? `/${t('property.perMonth')}` : '';
...
export function PropertyCard({ property, variant = 'grid' }: PropertyCardProps) {
  ...
  const priceStr = formatPrice(property.pricing.basePrice, property.pricing.currency, property.pricing.priceType, t);
  ...
  if (variant === 'list') {
    return (
      <article className="group flex overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md">
        {/* Image */}
        <Link href={`/properties/${property.id}`} className="relative w-48 shrink-0 sm:w-64" tabIndex={-1} aria-hidden="true">
        ...
        <div className="flex flex-1 flex-col justify-between p-4">
          <div>
            <div className="mb-1 flex items-start justify-between gap-2">
              <Link href={`/properties/${property.id}`} className="flex-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-1 rounded">
                <h3 className="font-semibold text-gray-900 line-clamp-2 hover:text-blue-600 transition-colors">{title}</h3>
              </Link>
              <button
                type="button"
                onClick={() => setSaved((v) => !v)}
                aria-label={saved ? t('property.saveProperty') : t('property.saveProperty')}
                aria-pressed={saved}
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              ...
              {property.specifications.beachView && (
                <Badge variant="success" size="sm">{t('property.beachView')}</Badge>
              )}
            </div>
          </div>
          <div className="mt-3 flex items-center justify-between">
            <div className="flex items-center gap-1 text-sm text-gray-600">
              ...
              <span className="text-gray-400">({property.ratings.count})</span>
            </div>
  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md">
      {/* Image */}
      <Link href={`/properties/${property.id}`} className="relative block overflow-hidden" tabIndex={-1} aria-hidden="true">
      ...
      <div className="flex flex-1 flex-col p-4">
        <Link href={`/properties/${property.id}`} className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-1 rounded">
          <h3 className="font-semibold text-gray-900 line-clamp-2 hover:text-blue-600 transition-colors">{title}</h3>
        </Link>
        <p className="mt-1 text-sm text-gray-500 line-clamp-1">{address}</p>
        {property.specifications.beachView && (
          <p className="mt-1 text-xs text-emerald-600 font-medium">{t('property.beachView')}</p>
        )}
        <div className="mt-auto pt-3 flex items-center justify-between">
          ...
          <p className="font-semibold text-gray-900 text-sm">{priceStr}</p>
        </div>
      </div>
    </article>
  );
