...
const stateClasses: Record<InputValidationState, string> = {
  default: "border-gray-300 focus:ring-0",
  error: "border-red-500 focus:border-red-500 focus:ring-0",
  success: "border-green-500 focus:border-green-500 focus:ring-0",
};
...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    const activeState: InputValidationState = errorText
      ? "error"
      : validationState;

    const describedBy =
      [errorText && errorId, helperText && helperId]
        .filter(Boolean)
        .join(" ") || undefined;
      ...
      <div className="flex flex-col gap-1">
        ...
        {errorText && (
          <p
            id={errorId}
            role="alert"
            className={["text-xs", stateIconColor.error].join(" ")}
          >
            {errorText}
          </p>
        )}
